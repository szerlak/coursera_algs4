...
//		}
	...
	private Node insertRecursive(Node node, Point2D p, boolean vertical,
		...
		if (node.vertical) {
			if (p.x() < node.point.x()) {
				RectHV left = new RectHV(rect.xmin(), rect.ymin(),
						node.point.x(), rect.ymax());
				node.left = insertRecursive(node.left, p, !node.vertical, left);
			} else {
				RectHV right = new RectHV(node.point.x(), rect.ymin(),
						rect.xmax(), rect.ymax());
				node.right = insertRecursive(node.right, p, !node.vertical,
						right);

			}
		} else {
			if (p.y() < node.point.y()) {
				RectHV down = new RectHV(rect.xmin(), rect.ymin(), rect.xmax(),
						node.point.y());
				node.left = insertRecursive(node.left, p, !node.vertical, down);
			} else {
				RectHV up = new RectHV(rect.xmin(), node.point.y(),
						rect.xmax(), rect.ymax());
